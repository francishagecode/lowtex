...
// `ao`; "Highlights" adds a white layer weighted by `1 − ao` (the inverse), so
// it brightens exposed surfaces and never lands in the crevices AO darkens.
...
/// Per-mesh-vertex smooth normal, computed on a *welded* copy of the mesh
/// (vertices sharing a position are merged, normals averaged). The mesh may store
/// split per-face normals; welding recovers cross-face smoothing so the AO
/// hemisphere is oriented by a smooth surface normal at hard edges.
